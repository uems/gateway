...
function Transformation(data) {
  var transformations = {
    ...
    'setCertificateName': function(person) {
      person.certificateName = data.newValue;
    },
    ...
    'issuedCertificate': function(person) {
      data.certificate.issueDate = data.when;
      person.issuedCertificate = data.certificate;
    },
    'setLoginHash': function(person) {
      person.loginHash = data.newValue;
    }
  };
...
function History() {
  ...
  this.setCertificateName = function(xid, name) {
    var entry = { xid: xid, kind: 'setCertificateName', newValue: name };
    return insert('transitions', entry);
  };
  ...
  this.setLoginHash = function(xid, hash, ip) {
    var entry = { xid: xid, kind: 'setLoginHash', newValue: hash, ip: ip };
    return insert('transitions', entry);
  };
  ...
  this.issuedCertificate = function(xid, certificate, url) {
    var entry = {
      xid: xid,
      kind: 'issuedCertificate',
      certificate: _(certificate).extend({ url: url })
    };
    return insert('transitions', entry);
  ...
  this.getXidByLoginHash = function(hash) {
    return db.transitions.findOne({ kind: 'setLoginHash', newValue: hash }).then(function(transition) {
      return transition.xid;
    });
  };
